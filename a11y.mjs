...
await page.evaluate(()=>{Object.assign(VillsporDebug.game.inventory,{wood:9,crystal:9,herb:9});VillsporDebug.game.reserve=[VillsporCore.makeCreature('lynglo',5)];VillsporDebug.game.caught.push('lynglo');VillsporDebug.game.team[0].level=5;VillsporDebug.game.team[0].bond=3;VillsporDebug.refreshPanels();});
await page.click('#teamBaseBtn');
violations.push(...await scan('basecamp'));
await page.click('#baseTeamBtn');
await page.evaluate(()=>VillsporDebug.chooseUnitForm('team',0,'spor'));
violations.push(...await scan('naturform'));
...
await page.evaluate(()=>{VillsporDebug.forceWorld();VillsporDebug.startTrainerBattle({id:'runa',name:'Stisøker Runa',enemy:'lynglo',level:4,reward:{wood:2,herb:2}});});
violations.push(...await scan('trenerkamp'));
assert.deepEqual(runtimeErrors,[]);
assert.deepEqual(violations,[]);
console.log(JSON.stringify({browser:browserName,states:7,violations:0},null,2));
await browser.close();