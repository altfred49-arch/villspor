...
const CREATURES={
  gnistrev:{id:'gnistrev',name:'Gnistrev',type:'ILD',role:'Rask sporjeger',bio:'En sky revegnist som lager små bål når den nyser.',trait:{name:'Første gnist',description:'+10 % skade over 80 % HP'},base:{hp:42,attack:14,defense:9,speed:14},moves:['glosprang','askesnurr','ildspor'],colors:['#f27b45','#ffd568','#6b3430'],shape:'fox'},
  mosemurr:{id:'mosemurr',name:'Mosemurr',type:'VEKST',role:'Seig beskytter',bio:'En lun skogsånd som sover under mosekledde steiner.',trait:{name:'Moselag',description:'8 % mindre skade over halv HP'},base:{hp:48,attack:11,defense:14,speed:8},moves:['rotgrep','sporevind','mosevern'],colors:['#6da85c','#b7d875','#355e45'],shape:'moss'},
  bekkskvett:{id:'bekkskvett',name:'Bekkskvett',type:'VANN',role:'Stødig helbreder',bio:'En rastløs bekkunge som alltid svømmer motstrøms.',trait:{name:'Kildepuls',description:'Helbredelse gir 18 % HP'},base:{hp:44,attack:13,defense:10,speed:13},moves:['skumsprut','duggslor','kildepust'],colors:['#55bdd3','#d7f4e9','#28617d'],shape:'otter'},
  kongleklo:{id:'kongleklo',name:'Kongleklo',type:'VEKST',role:'Herdet taktiker',bio:'Samler blanke ting i kongleskallet sitt.',trait:{name:'Hardt skall',description:'10 % mindre skade under halv HP'},base:{hp:50,attack:14,defense:13,speed:7},moves:['konglekast','barkbrak','harpikssnor'],colors:['#8c6741','#79ad52','#403b2d'],shape:'crab'},
  fjordfnugg:{id:'fjordfnugg',name:'Fjordfnugg',type:'VANN',role:'Rask tåkevokter',bio:'Et fjærlett fjordvesen som varsler regn.',trait:{name:'Medvind',description:'+8 % skade når den er raskest'},base:{hp:40,attack:12,defense:9,speed:17},moves:['bobleblink','fossedunk','taakeskjold'],colors:['#91dbe4','#f0fff3','#527c9c'],shape:'bird'},
  kullvinge:{id:'kullvinge',name:'Kullvinge',type:'ILD',role:'Snikende svekker',bio:'Trives i varme bergsprekker og tegner med sot.',trait:{name:'Sotmerke',description:'Svekkelse varer én ekstra runde'},base:{hp:43,attack:15,defense:8,speed:15},moves:['kullnebb','vingegnist','sotdis'],colors:['#332f3e','#f16a42','#ffc85a'],shape:'bat'},
  flammefyrsten:{id:'flammefyrsten',name:'Flammefyrsten',type:'ILD',role:'Kraftig fjellvokter',bio:'Fjellets stolte, eldgamle vokter. Et vennskap som må fortjenes.',trait:{name:'Glødekrone',description:'10 % mindre skade fra typefordel'},base:{hp:64,attack:16,defense:14,speed:11},moves:['flammesverd','lavastorm','glodhjerte'],colors:['#9d2e28','#ff6b32','#ffd45a'],shape:'dragon',boss:true,legendary:true}
};
...
function traitDamageFactor(attacker,defender,mult){let factor=1;const as=statsFor(attacker.id,attacker.level),ds=statsFor(defender.id,defender.level);if(attacker.id==='gnistrev'&&attacker.hp/attacker.maxHp>.8)factor*=1.1;if(attacker.id==='fjordfnugg'&&as.speed>ds.speed)factor*=1.08;if(defender.id==='mosemurr'&&defender.hp/defender.maxHp>.5)factor*=.92;if(defender.id==='kongleklo'&&defender.hp/defender.maxHp<.5)factor*=.9;if(defender.id==='flammefyrsten'&&mult>1)factor*=.9;return factor;}
function calcDamage(attacker,defender,moveId,roll=.85){const move=MOVES[moveId];const a=statsFor(attacker.id,attacker.level);const d=statsFor(defender.id,defender.level);const weaken=attacker.status?.weaken?0.78:1;const guard=defender.status?.guard?0.72:1;const mult=typeMultiplier(move.type,CREATURES[defender.id].type);const raw=(move.power+a.attack*.72-d.defense*.36)*clamp(roll,.75,1)*mult*weaken*guard*traitDamageFactor(attacker,defender,mult);return{damage:Math.max(2,Math.round(raw)),mult,move};}
function applyMove(attacker,defender,moveId,roll=.85){const result=calcDamage(attacker,defender,moveId,roll);defender.hp=clamp(defender.hp-result.damage,0,defender.maxHp);const effect=result.move.effect;if(effect==='heal')attacker.hp=clamp(attacker.hp+Math.round(attacker.maxHp*(attacker.id==='bekkskvett'?.18:.14)),0,attacker.maxHp);if(effect==='guard')attacker.status.guard=2;if(effect==='weaken')defender.status.weaken=attacker.id==='kullvinge'?3:2;return result;}
...
function validateData(){const errors=[];const ids=Object.keys(CREATURES);if(ids.filter(id=>!CREATURES[id].boss).length!==6)errors.push('Må ha seks fangbare vesener');for(const [id,c] of Object.entries(CREATURES)){if(!TYPES[c.type])errors.push(id+' har ugyldig type');if(!c.trait?.name||!c.trait?.description)errors.push(id+' mangler egenskap');if(c.moves.length<2)errors.push(id+' mangler angrep');for(const m of c.moves)if(!MOVES[m])errors.push(id+' viser til ukjent angrep '+m);}return errors;}
